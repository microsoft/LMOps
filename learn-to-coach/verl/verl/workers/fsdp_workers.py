...
class ActorRolloutRefWorker(Worker, DistProfilerExtension):
    ...
    @register(dispatch_mode=Dispatch.ONE_TO_ALL)
    @DistProfiler.annotate(color="red")
    def generate_sequences_textgame(
        self,
        env_config,
        env_num,
        experiences,
        ...
        if previous_attempts is not None:
            assert len(previous_attempts) == env_num, (
                f"previous_attempts length ({len(previous_attempts)}) must match env_num ({env_num})"
            )
        ...
        local_previous_attempts = (
            previous_attempts[start_idx:end_idx]
            if previous_attempts is not None and local_env_num > 0
            else None
        )
        ...
        local_output = self._generate_sequences_textgame_impl(
            ...
            seeds=local_seeds,
            validate=validate,
            previous_attempts=local_previous_attempts,
        )
    ...
    def _generate_sequences_textgame_impl(
        self,
        env_config,
        env_num,
        experiences,
