...
class DataParallelPPOActor(BasePPOActor):
    ...
    def _update_policy_l2l(self, data, temperature):
        """Standard GRPO/PPO policy gradient update for L2L exp_learner."""
        ...
        if self.config.use_dynamic_bsz:
            ...
            micro_batches = batch.split(actual_ppo_mini_batch_size)
        ...
        for epoch in range(self.config.ppo_epochs):
            ...
            for data in micro_batches:
                ...
                old_log_probs = data["old_log_probs"][:, :response_length]
                advantages = data["advantages"][:, :response_length]

                entropy, log_prob = self._forward_micro_batch(
                    micro_batch=data, temperature=temperature,
                    calculate_entropy=True, return_all_logits=False
                )

                # Standard PPO/GRPO policy loss
                pg_loss, pg_clipfrac, ppo_kl, pg_clipfrac_lower = compute_policy_loss(
                    old_log_prob=old_log_probs,
                    log_prob=log_prob,
                    advantages=advantages,
                    response_mask=response_mask,
                    cliprange=self.config.get("clip_ratio", 0.2),
                )
                entropy_agg = agg_loss(loss_mat=entropy, loss_mask=response_mask, loss_agg_mode=self.config.loss_agg_mode)

                if self.config.use_dynamic_bsz:
                    loss = pg_loss * (responses.shape[0] / actual_ppo_mini_batch_size)
                else:
                    loss = pg_loss / (actual_ppo_mini_batch_size // self.config.ppo_micro_batch_size_per_gpu)
                loss.backward()
    ...
    def update_policy(self, data: DataProto):
        ...
        # L2L mode: standard GRPO/PPO update using advantages + old_log_probs
        if data.meta_info.get("l2l_mode", False):
            return self._update_policy_l2l(data, temperature)

        # OPCD mode: KL distillation (consolidate stage)
