...
def _make_textgame_envs(env_id: str, num_players: int, env_num: int, size: int, num_holes: int, randomize_start_goal: bool, num_boxes: int, dim_room: tuple, seeds: List[int] = None):
    ...
    """
    envs = []
    import textarena as ta  # lazy import: only needed for textgame mode
    if seeds is None:
...
def _strip_env_rules_from_observation(observation_str: str) -> str:
    """Strip the first-step env rules block (Sokoban full rules / Frozen Lake
    intro) and replace it with a short action-format reminder. Shared by both
    the L2L textgame prompt and the self-refine textgame prompt so any future
    edit to the rules-strip lands in one place."""
    if "Sokoban" in observation_str:
        return observation_str.replace("You are solving the Sokoban puzzle. You are the player and you need to push all boxes to targets.\n        When you are right next to a box, you can push it by moving in the same direction.\n        You cannot push a box through a wall, and you cannot pull a box.\n        On the board, objects are represented as: \n        - The player (you) appears as 'P' \n        - Walls are represented with '#' \n        - Boxes are marked as 'X' \n        - Empty goals are shown with a 'O'\n        - Boxes on goals are visualized with '√'\n        You can also use [w] for up, [a] for left, [s] for down, and [d] for right.",
...
def _build_textgame_prompt(experience: str, observation_str: str) -> str:
    ...
    observation_no_rules = _strip_env_rules_from_observation(observation_str)
    if experience:
        prompt_text = (
...
def _build_textgame_refine_prompt(previous_attempt: str, observation_str: str) -> str:
    """Self-refine textgame prompt: re-play the same env (same seed) after
    seeing the agent's full previous trajectory. Caller is responsible for
    formatting `previous_attempt` as the multi-round obs+response log followed
    by the env's final feedback (Success/Failure + reason).

    The same env-rules block is stripped from the *current* observation (so
    the agent isn't told the rules twice — the prologue inside
    `previous_attempt` already carries them), then both blocks are placed in
    the prompt in their natural order: previous attempt first, current
    situation second."""
    observation_no_rules = _strip_env_rules_from_observation(observation_str)
    return (
        "You are an agent playing a game on a grid, acting as a reasoning engine.\n\n"
        "Here is your previous attempt at this game:\n"
        f"{previous_attempt}\n\n"
        "Review your previous attempt above, identify any errors or improvements, "
        "then play the game again from the beginning.\n\n"
        f"Current situation:\n{observation_no_rules}\n\n"
        "What action do you take? (Remember to wrap your final answer of the action in square brackets)"
    )
...
class vLLMRollout(BaseRollout):
    ...
    @torch.no_grad()
    def generate_sequences_textgame(self, env_config, env_num, experiences, tokenizer, num_steps=50, seeds: List[int] = None, validate: bool = False, previous_attempts: List[str] = None):
        """
        ...
        if previous_attempts is not None:
            assert len(previous_attempts) == len(envs), f"Number of previous_attempts ({len(previous_attempts)}) must match number of envs ({len(envs)})"
        ...
        while True:
            ...
            for env_idx in active_env_indices:
                ...
                if previous_attempts is not None and previous_attempts[env_idx]:
                    prompt_text = _build_textgame_refine_prompt(previous_attempts[env_idx], observation_str)
                else:
