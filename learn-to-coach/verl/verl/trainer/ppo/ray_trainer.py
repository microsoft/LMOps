...
from verl.trainer.ppo.l2c import L2CCrossInstanceMixin
from verl.trainer.ppo.l2c_eval import L2CEvaluationMixin
from verl.trainer.ppo.l2c_train import L2CTrainingMixin
...
class RayPPOTrainer(L2CTrainingMixin, L2CEvaluationMixin, L2CCrossInstanceMixin):
    ...
    def _validate_config(self):
        ...
        if config.trainer.stage in ("l2l", "l2l_eval"):
            self.l2c_mode = resolve_l2c_mode(config.trainer, config.trainer.stage)
            if (
                config.trainer.stage == "l2l"
                and int(config.actor_rollout_ref.rollout.n) < 2
            ):
                raise ValueError(
                    "L2C training needs actor_rollout_ref.rollout.n >= 2 "
                    "to form a non-degenerate GRPO candidate group"
                )
            compute_loss_metrics = bool(config.trainer.get("compute_loss_metrics", False))
            if compute_loss_metrics and (
                config.trainer.stage != "l2l_eval"
                or self.is_textgame
                or self.l2c_mode.reward_scope is not L2CRewardScope.SAME_INSTANCE
                or self.l2c_mode.coaching_rounds != 1
            ):
                raise ValueError(
                    "trainer.compute_loss_metrics is implemented only for "
                    "single-round same-instance math eval"
                )
    ...
    def _truncate_experience(self, experience, max_tokens):
        ...
        def is_marker(line):
            ...
            elif self.config.trainer.prompt_version == 'v1':
                return line.strip().startswith("- EXPERIENCE ITEM:")
            elif self.config.trainer.prompt_version in ['v3', 'v4', 'v5']:
                return True
            return False
    ...
    def _validate_system_prompt(self):
        ...
        if self.config.trainer.system_prompt_type == 'medmcqa':
            ...
            accs = get_safety_acc(eval_batch, self.tokenizer, self.config.trainer.system_prompt_version)
        ...
        for idx in sample_indices:
            ...
            saved_samples.append({
                ...
                "gt": eval_batch[idx].non_tensor_batch["answer"],
                "acc": accs[idx],
            })
    ...
    def init_workers(self):
        ...
        requires_exp_learner = self.config.trainer.stage in ("l2l", "l2l_eval")
    ...
    def _load_checkpoint(self):
        if self.config.trainer.resume_mode == "disable":
            # 1-node eval with merged HF exp_learner: no FSDP shards to load,
            # but caller wants global_step to reflect the ckpt step (for wandb
            # logging, dump dir naming). Pass --force_global_step via hydra.
            force_step = OmegaConf.select(self.config.trainer, "force_global_step", default=None)
            if force_step is not None:
                self.global_steps = int(force_step)
                print(f"[L2L Eval] disable mode + force_global_step={self.global_steps}")
            return 0
        ...
        # L2L training only saves exp_learner (actor stays frozen at base model),
        # so for l2l_eval we must NOT try to load a non-existent actor/ subdir.
        # The actor worker was already initialized from model.path in __init__.
        if self.config.trainer.stage == "l2l_eval":
            ...
            print(f"[L2L Eval] Loaded exp_learner from {exp_learner_path}; actor kept at base model")
            return self.global_steps
        # Mirror _save_checkpoint: L2C freezes the actor and stores only the
        # coach (exp_learner). Consolidation is the only stage with actor shards.
        if self.config.trainer.stage == "consolidate":
