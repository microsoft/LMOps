...
class ActorRolloutRefWorker(Worker, DistProfilerExtension):
    ...
    def init_model(self):
        ...
        if self._is_ref and not self._is_actor:
            # Create a checkpoint manager for ref policy to support iterative teacher update
            checkpoint_contents = OmegaConf.create({"load_contents": ["model"], "save_contents": []})
            self.checkpoint_manager = FSDPCheckpointManager(
    ...
    def compute_ref_log_prob(self, data: DataProto):
        ...
        # Extract ref lm_head weight if hidden states were returned
        if data.meta_info.get("return_hidden_states", False) and hasattr(self.ref_policy, '_cached_lm_head_weight'):
            output.meta_info["ref_lm_head_weight"] = self.ref_policy._cached_lm_head_weight.cpu()
            del self.ref_policy._cached_lm_head_weight
    ...
    def load_checkpoint(self, local_path, hdfs_path=None, del_local_after_load=False):
        assert self._is_actor or self._is_ref or (not self._is_actor and self._is_rollout), f"Checkpoint loading is only supported for Actor, Ref, or standalone Rollout Workers, but got is_actor={self._is_actor}, is_ref={self._is_ref}, is_rollout={self._is_rollout}"

        if self._is_offload_param:
            load_fsdp_model_to_gpu(self.actor_module_fsdp if self._is_actor else self.ref_module_fsdp)
        ...
        if self._is_offload_param:
            offload_fsdp_model_to_cpu(self.actor_module_fsdp if self._is_actor else self.ref_module_fsdp)
