        ...
        class _MyInferenceEngineProxy():
            ...
            def __init__(self):
                def _wake_up_func_impl():
                    ...
                    try:
                        import vllm.worker.worker as _vw
                        if hasattr(_vw.Worker, '_assert_memory_footprint_increased_during_profiling'):
                            _vw.Worker._assert_memory_footprint_increased_during_profiling = lambda self: None
            ...
            def sleep(self, *args, **kwargs):
                if self.engine is not None:
                    import gc
                    # vLLM >= 0.11: V1 engine doesn't free GPU memory on del.
                    # Manually release KV cache + model params via storage().resize_(0).
                    # Skip buffers (e.g. rotary cos_sin_cache) — they're small and
                    # recomputed on next model init.
                    import vllm as _vllm_mod
                    _vv = getattr(_vllm_mod, '__version__', '')
                    if any(_vv.startswith(v) for v in ("0.11.1", "0.11.2", "0.12.0")):
                        try:
                            engine_core = self.engine.llm_engine.engine_core
