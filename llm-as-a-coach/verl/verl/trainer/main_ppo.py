...
class TaskRunner:
    def run(self, config):
        ...
        # Add a static ref policy worker for multidata_fixed_teacher (never updated by iter_teacher)
        multidata_fixed_teacher = config.trainer.get("multidata_fixed_teacher", None)
        if multidata_fixed_teacher is not None:
            role_worker_mapping[Role.RefPolicyStatic] = ray.remote(ActorRolloutRefWorker)
            mapping[Role.RefPolicyStatic] = global_pool_id
