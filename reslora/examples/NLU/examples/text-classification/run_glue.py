...
import torch
import wandb
from peft import get_peft_model, AdaLoraConfig, TaskType, LoraConfig, PromptEncoderConfig
...
from transformers import (
    ...
    set_seed, TrainerState, TrainerControl,
)
...
def main():
    ...
    training_args.use_deterministic_algorithms = True
    training_args.cls_dropout = False
    ...
    #     model.print_trainable_parameters()
        ...
        resmodel = ResLoraModel(model, resconfig, epochs=training_args.num_train_epochs, model_name='roberta')
        logger.info(
    ...
    class ResTrainer(Trainer):
        ...
        def evaluate(self, *args, **kwargs) -> Dict[str, float]:
            if (
                    self.resmodel.resconfig.res_flag == 1 or self.resmodel.resconfig.res_flag == 3) and self.resmodel.resconfig.merge_flag > 0:
    ...
    if model_args.method == "reslora":
        ...
        if global_rank == 0:
            if data_args.debug_flag:
                ...
                wandb.init(project=f"reslora_glue", tags=tags, name="_".join(tags))
