...
from data_utils.lm_datasets import LMTrainDataset, LMPT_loader
...
def get_distil_loss(args, tokenizer, model, teacher_model, model_batch, no_model_batch, logits):
    ...
    if not args.seqKD_DPO:
        if args.model_parallel:
            ...
            prod_probs = torch.masked_fill(teacher_probs * logprobs, inf_mask, 0) # [bs, max_lenth, dim]
        ...
        lenths_ys = find_non_repeating_lengths(y_s)
        lenths_ys = lenths_ys / 256
        ...
        stu_logratios = student_chosen_logps - student_reject_logps # [bs, maxlenth]
        ...
        if args.ipo:
            ...
            # simpo
            if args.simpo:
                distil_loss = distil_loss / lenths_ys.unsqueeze(1)
...
def get_lmpt_loss(args, tokenizer, model, teacher_model, batch, logits):
    ...
    distil_loss = 0
    if teacher_model is not None and args.kd_ratio is not None:
        with torch.no_grad():
...
def finetune(args, tokenizer: AutoTokenizer, model: deepspeed.DeepSpeedEngine, optimizer: AdamW, lr_scheduler, dataset, device, teacher_model=None, lmpt = None):
    print_rank("Start Fine-tuning")
    if lmpt is not None:
        lmpt_dataloader = lmpt.create_loader(
            args.batch_size, shuffle=True, num_workers=args.num_workers, drop_last=True)
        lmpt_iterator = iter(lmpt_dataloader)
    ...
    eval_result = {'avg_loss': 0, 'exact_match': 0, 'rougeL': 0}
    ...
    for epoch in range(args.epochs):
        ...
        for it, (model_batch, no_model_batch, gen_data) in enumerate(train_dataloader):
            if lmpt is not None:
                try:
                    lm_batch = next(lmpt_iterator)
                except StopIteration:
                    lm_epochs += 1
                    lmpt_dataloader.sampler.set_epoch(lm_epochs)
                    lmpt_iterator = iter(lmpt_dataloader)
                    lm_batch = next(lmpt_iterator)
                lmpt.move_to_device(*lm_batch, device)
                dataset["train"].move_to_device(model_batch, no_model_batch, gen_data, device)
                input_batch = {}
                for k in model_batch:
                    input_batch[k] = torch.cat([model_batch[k], lm_batch[0][k]], dim=0)
                dataset["train"].move_to_device(input_batch, no_model_batch, gen_data, device)

                torch.cuda.synchronize()
                st_time = time.time()
                outputs = model(**input_batch, use_cache=False)
                full_logits = outputs.logits 
                logits = full_logits[:model_batch['input_ids'].size(0)] 
                lmpt_logits = full_logits[model_batch['input_ids'].size(0):]
            else:
                dataset["train"].move_to_device(model_batch, no_model_batch, gen_data, device)
                torch.cuda.synchronize()
                st_time = time.time()
            ...
            if lmpt is not None:
                lmpt_loss = get_lmpt_loss(args, tokenizer, model, teacher_model, lm_batch, lmpt_logits)
                loss = loss + args.lm_coef * lmpt_loss
            model.backward(loss)
            model.step()
...
def main():
    ...
    if args.LMPTdata:
        lmpt = LMPT_loader(args, tokenizer, "train", args.lm_data_dir, num=args.train_num)
    else:
        lmpt = None
    if args.do_train:
        model = finetune(args, tokenizer, model, optimizer, lr_scheduler, dataset, device, teacher_model=teacher_model, lmpt = lmpt)
