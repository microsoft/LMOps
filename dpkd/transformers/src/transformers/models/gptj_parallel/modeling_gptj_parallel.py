...
class ParallelGPTJAttention(nn.Module):
    def __init__(self, config):
        ...
        self.num_heads_per_partition = divide(self.num_attention_heads, self.world_size) # n_p
    ...
    def forward(
        ...
        query = self._split_heads(query, self.num_heads_per_partition, self.head_dim, True)
        key = self._split_heads(key, self.num_heads_per_partition, self.head_dim, True)
        value = self._split_heads(value, self.num_heads_per_partition, self.head_dim, False)
...
class ParallelGPTJMLP(nn.Module):
...
class ParallelGPTJBlock(nn.Module):
    def __init__(self, config):
        super().__init__()
        inner_dim = config.n_inner if config.n_inner is not None else 4 * config.n_embd
        self.ln_1 = nn.LayerNorm(config.n_embd, eps=config.layer_norm_epsilon)
        self.attn = ParallelGPTJAttention(config)
        self.mlp = ParallelGPTJMLP(inner_dim, config)
...
class ParallelGPTJPreTrainedModel(PreTrainedModel):
    """
    ...
    def _set_gradient_checkpointing(self, module, value=False):
        if isinstance(module, ParallelGPTJModel):
            module.gradient_checkpointing = value


class ParallelGPTJModel(ParallelGPTJPreTrainedModel):
    def __init__(self, config):
        ...
        self.h = nn.ModuleList([ParallelGPTJBlock(config) for _ in range(config.n_layer)])
...
class ParallelGPTJForCausalLM(ParallelGPTJPreTrainedModel):
    _keys_to_ignore_on_load_missing = [r"h\.\d+\.attn\.masked_bias", r"h\.\d+\.attn\.bias"]

    def __init__(self, config):
        super().__init__(config)
        self.transformer = ParallelGPTJModel(config)
        # self.lm_head = nn.Linear(config.n_embd, config.vocab_size)
        # self.lm_head = VocabParallelEmbedding(config.n_embd, config.vocab_size)
    ...
    def forward(
        ...
        logits_parallel = self.lm_head(hidden_state_parallel)
        lm_logits = logits_parallel.to(torch.float32)
        loss = None
