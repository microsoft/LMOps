...
class ParallelQWenAttention(nn.Module):
    def __init__(self, config):
        ...
        self.hidden_size_per_partion = divide(self.hidden_size, self.world_size)
        self.num_heads_per_partion = divide(self.num_heads, self.world_size)
        ...
        self.c_attn = ColumnParallelLinear(config.hidden_size,
    ...
    def forward(
        ...
        query, key, value = mixed_x_layer.split(self.hidden_size_per_partion, dim=2)

        query = self._split_heads(query, self.num_heads_per_partion, self.head_dim)
        key = self._split_heads(key, self.num_heads_per_partion, self.head_dim)
        value = self._split_heads(value, self.num_heads_per_partion, self.head_dim)
        ...
        # )
        context_layer = self._merge_heads(
            attn_output, self.num_heads_per_partion, self.head_dim
        )
...
class ParallelQWenMLP(nn.Module):
    def __init__(self, config):
        super().__init__()
        # self.w1 = nn.Linear(
...
class ParallelQWenBlock(nn.Module):
    def __init__(self, config):
        super().__init__()
        ...
        self.attn = ParallelQWenAttention(config)
        self.ln_2 = RMSNorm(
            hidden_size,
            eps=config.layer_norm_epsilon,
        )

        self.mlp = ParallelQWenMLP(config)
...
class ParallelQWenPreTrainedModel(PreTrainedModel):
...
class ParallelQWenModel(ParallelQWenPreTrainedModel):
...
class ParallelQWenLMHeadModel(ParallelQWenPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        self.transformer = ParallelQWenModel(config)
    ...
    def forward(
        ...
        hidden_state_parallel = copy_to_model_parallel_region(transformer_outputs[0])
