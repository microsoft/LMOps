...
class ParallelGPT2Attention(nn.Module):
    def __init__(self, config, is_cross_attention=False, layer_idx=None):
        ...
        self.embed_dim_per_partition = divide(self.embed_dim, self.world_size) # h_p
    ...
    def forward(
        ...
        if encoder_hidden_states is not None:
            ...
            key, value = self.c_attn(encoder_hidden_states).split(self.embed_dim_per_partition, dim=2)
            attention_mask = encoder_attention_mask
        else:
            query, key, value = self.c_attn(hidden_states).split(self.embed_dim_per_partition, dim=2)

        query = self._split_heads(query, self.num_heads_per_partition, self.head_dim)
        key = self._split_heads(key, self.num_heads_per_partition, self.head_dim)
        value = self._split_heads(value, self.num_heads_per_partition, self.head_dim)
        ...
        attn_output = self._merge_heads(attn_output, self.num_heads_per_partition, self.head_dim)
...
class ParallelGPT2Block(nn.Module):
    def __init__(self, config, layer_idx=None):
        ...
        self.attn = ParallelGPT2Attention(config, layer_idx=layer_idx)
        self.ln_2 = nn.LayerNorm(hidden_size, eps=config.layer_norm_epsilon)

        if config.add_cross_attention:
            self.crossattention = ParallelGPT2Attention(config, is_cross_attention=True, layer_idx=layer_idx)
            self.ln_cross_attn = nn.LayerNorm(hidden_size, eps=config.layer_norm_epsilon)

        self.mlp = ParallelGPT2MLP(inner_dim, config)
...
class ParallelGPT2PreTrainedModel(PreTrainedModel):
    """
    ...
    def _set_gradient_checkpointing(self, module, value=False):
        if isinstance(module, ParallelGPT2Model):
...
class ParallelGPT2Model(ParallelGPT2PreTrainedModel):
    def __init__(self, config):
        ...
        self.h = nn.ModuleList([ParallelGPT2Block(config, layer_idx=i) for i in range(config.num_hidden_layers)])
...
class ParallelGPT2LMHeadModel(ParallelGPT2PreTrainedModel):
...
class GPT2DoubleHeadsModel(ParallelGPT2PreTrainedModel):
    _tied_weights_keys = ["lm_head.weight"]
...
class GPT2ForSequenceClassification(ParallelGPT2PreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
...
class GPT2ForTokenClassification(ParallelGPT2PreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
        self.num_labels = config.num_labels
...
class GPT2ForQuestionAnswering(ParallelGPT2PreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
        self.num_labels = config.num_labels
        self.transformer = ParallelGPT2Model(config)
