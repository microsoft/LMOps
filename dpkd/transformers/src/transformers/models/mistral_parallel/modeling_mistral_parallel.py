...
class MistralFlashAttention2(ParallelMistralAttention):
    """
...
class MistralDecoderLayer(nn.Module):
    def __init__(self, config: MistralConfig):
        ...
        self.hidden_size = config.hidden_size
        self.self_attn = (
            ParallelMistralAttention(config=config)
            if not getattr(config, "_flash_attn_2_enabled", False)
            else MistralFlashAttention2(config)
        )
        self.mlp = ParallelMistralMLP(config)
...
class ParallelMistralPreTrainedModel(PreTrainedModel):
...
class ParallelMistralModel(ParallelMistralPreTrainedModel):
    """
...
class ParallelMistralForCausalLM(ParallelMistralPreTrainedModel):
    _tied_weights_keys = ["lm_head.weight"]

    def __init__(self, config):
        super().__init__(config)
        self.model = ParallelMistralModel(config)
        self.vocab_size = config.vocab_size
...
class MistralForSequenceClassification(ParallelMistralPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
