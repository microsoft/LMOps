...
class ParallelLlamaAttention(nn.Module):
    ...
    def __init__(self, config: LlamaConfig):
        ...
        self.num_key_value_heads_per_partition = divide(self.num_key_value_heads, self.world_size) # n_kv_p
    ...
    def forward(
        ...
        **kwargs,
        ...
        query_states = query_states.view(bsz, q_len, self.num_heads_per_partition, self.head_dim).transpose(1, 2)
...
class LlamaFlashAttention2(ParallelLlamaAttention):
    """
...
class ParallelLlamaDecoderLayer(nn.Module):
    def __init__(self, config: LlamaConfig):
        ...
        self.hidden_size = config.hidden_size
        self.self_attn = (
            ParallelLlamaAttention(config=config)
...
class ParallelLlamaModel(ParallelLlamaPreTrainedModel):
    ...
    def __init__(self, config: LlamaConfig):
        ...
        self.layers = nn.ModuleList([ParallelLlamaDecoderLayer(config) for i in range(config.num_hidden_layers)])
...
class LlamaForSequenceClassification(ParallelLlamaPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
