...
def build_alibi_tensor(attention_mask: torch.Tensor, n_head: int, dtype, device, past_key_values_length: int, prefix_parallel: int = None) -> torch.Tensor:
    ...
    if prefix_parallel is not None and prefix_parallel > 1:
        ...
        arange_tensor = torch.cat((arange_tensor, 
        torch.arange(real_past_key_values_length, real_past_key_values_length + current_length)), dim=0)
        arange_tensor = arange_tensor.to(device) * attention_mask[:, None]
    else:
...
class BloomAttention(nn.Module):
    ...
    def forward(
        ...
        if prefix_parallel is not None and prefix_parallel > 1:
            ...
            attn_weights = torch.cat((attn_weights, attn_weights[:, :, :, past_key_values_length:].repeat(1, 1, 1, (prefix_parallel - 1))), dim=-1)
            attn_weights = nn.functional.softmax(attn_weights, dim=-1, dtype=torch.float32).to(input_dtype)
            attn_weights = torch.cat((attn_weights[:, :, :, :past_key_values_length], \
                attn_weights[:, :, :, past_key_values_length:].view(attn_weights.shape[0], attn_weights.shape[1], attn_weights.shape[2], prefix_parallel, -1).sum(dim=-2)), dim=-1)   
        else:
            attn_weights = nn.functional.softmax(attn_weights, dim=-1, dtype=torch.float32).to(input_dtype)
...
class BloomBlock(nn.Module):
    ...
    def forward(
        ...
        attn_outputs = self.self_attention(
            ...
            prefix_parallel=prefix_parallel,
        )
