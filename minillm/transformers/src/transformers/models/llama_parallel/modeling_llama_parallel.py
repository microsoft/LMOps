...
class ParallelLlamaAttention(nn.Module):
    ...
    def __init__(self, config: LlamaConfig):
        ...
        self.hidden_size_per_partition = divide(self.hidden_size, self.world_size) # h_p
    ...
    def forward(
        ...
        query_states = self.q_proj(hidden_states).view(bsz, q_len, self.num_heads_per_partition, self.head_dim).transpose(1, 2)
        key_states = self.k_proj(hidden_states).view(bsz, q_len, self.num_heads_per_partition, self.head_dim).transpose(1, 2)
        value_states = self.v_proj(hidden_states).view(bsz, q_len, self.num_heads_per_partition, self.head_dim).transpose(1, 2)
        ...
        if attn_weights.size() != (bsz, self.num_heads_per_partition, q_len, kv_seq_len):
            raise ValueError(
                f"Attention weights should be of size {(bsz * self.num_heads_per_partition, q_len, kv_seq_len)}, but is"
                f" {attn_weights.size()}"
            )
        ...
        if attn_output.size() != (bsz, self.num_heads_per_partition, q_len, self.head_dim):
            raise ValueError(
                f"`attn_output` should be of size {(bsz, self.num_heads_per_partition, q_len, self.head_dim)}, but is"
                f" {attn_output.size()}"
            )
...
class ParallelLlamaDecoderLayer(nn.Module):
    def __init__(self, config: LlamaConfig, drop_path_rate: float = 0.0):
        super().__init__()
        self.hidden_size = config.hidden_size
        self.self_attn = ParallelLlamaAttention(config=config)
        self.mlp = ParallelLlamaMLP(
...
class ParallelLlamaPreTrainedModel(PreTrainedModel):
    ...
    def _set_gradient_checkpointing(self, module, value=False):
        if isinstance(module, ParallelLlamaModel):
            module.gradient_checkpointing = value


class ParallelLlamaModel(ParallelLlamaPreTrainedModel):
    """
    ...
    def __init__(self, config: LlamaConfig):
        ...
        self.layers = nn.ModuleList([ParallelLlamaDecoderLayer(config, drop_path_rate=dpr[i]) for i in range(config.num_hidden_layers)])
...
class ParallelLlamaForCausalLM(ParallelLlamaPreTrainedModel):
