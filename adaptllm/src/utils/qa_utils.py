...
def parse_ner(text):
    ...
    output = []
    for y in x:
      name = y[0][:-1] if len(y[0])>1 else ''
      entity = y[1][1:-1] if len(y[1])>2 else ''
      if Entity(name,entity,0) not in output:
          output.append(Entity(name,entity,0))
    return output

def normalized_num(num_str):
  def eval_num(num):
  ...
  num_set = regex.findall(num_str)
  return [eval_num(num) for num in num_set]